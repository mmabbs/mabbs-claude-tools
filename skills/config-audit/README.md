...
1. Loads eight criteria docs that define what earns its keep in each config layer and the boundary tests between adjacent layers.
...
3. Spawns a single reasoning agent (on Opus, max effort) that reads all inventories plus all criteria docs and produces findings.
...
2. **Boundary opinions.** The criteria docs encode opinionated defaults — notably "when the memory/rule boundary is fuzzy, resolve to memory." If you prefer more rules and less memory, edit `references/criteria-memory.md` and the reasoning rules in `SKILL.md`.
3. **Platform.** The findings-file `open` command is macOS; substitute `xdg-open` on Linux.
...
| File | Purpose |
| --- | --- |
| `SKILL.md` | The skill: arguments, scan phases, agent prompts, output formats |
